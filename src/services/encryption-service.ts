...
async function encrypt(secret: string, encryptKey: string): Promise<string> {
  ...
  const ciphertext = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, messageUtf8)
...
async function decrypt(encryptedSecret: string, encryptKey: string): Promise<string | null> {
...
export { decrypt, encrypt, hashPassword }
